...
export class BaseRepository<T> implements iBaseRepository<T> {
    ...
    async findById(id: number): Promise<T[]>{
        const queryResult=await db.query(`SELECT * FROM ${this.tableName} WHERE id = $1`, [id]);
    ...
    async findByField(fields: string[], condition: Record<string, any>, logicOperator: string): Promise<T[]> {
        ...
        const queryResult = await db.query(`SELECT ${field} FROM ${this.tableName} WHERE ${mappedConditions}`);
    ...
    async updateData(datas: Record<string, any>, conditions: Record<string, any>, logicOperator: string): Promise<QueryResult<any>> {
        ...
        const updatedData = await db.query(querySyntax, toUpdateDatas);
        return updatedData;
    }
